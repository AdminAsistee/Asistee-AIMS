...
class TodayCleaning extends Component {
    constructor() {
        super();
        this.calendarFormat = {
    ...
    render() {
        const cleanings = get(this.props, 'dashboard.unassigned_cleanings', []);

        return (
            <div className="fl w-100 ba b--black-10 bg-white br2 pa2 pa3-l">
                <h2 className='fl fw2 mv2 black-90 fl w-100 pl2'>Unassigned Cleanings</h2>
                <div className='fl w5 bb bw1 b--black-10 ml2'/>

                {(get(this.props, 'dashboard.unassigned_cleanings_loading') && !get(this.props, 'dashboard.unassigned_cleanings_loaded')) ?
                    <div className="fl w-100"><LoaderCenter/></div> :
                    <FlipMove className="w-100 pt3 dt">
                        <div className="dt-row">
                            <div className="fw5 underline dtc ph2 pv3 v-top w-70">Cleaning Location</div>
                            <div className="fw5 underline dtc ttc ph2 pv3 v-top w-30">Cleaner / Info</div>
                        </div>
                        {cleanings.map((cleaning, i) => (
                            <div className="dt-row striped--near-white" key={cleaning.id}>
                                ...
                                <div className="dtc ttc pa2 v-top">
                                    <AssignMeButton cleaning={cleaning}/>
                ...
                {/*<pre>*/}
                {/*{JSON.stringify(get(this.props.dashboard, 'unassigned_cleanings'), null, 4)}*/}
                {/*</pre>*/}
                {/*</div>*/}
            </div>
        );
...
// connect and export
export default connect(mapStateToProps, mapDispatchToProps)(TodayCleaning);
